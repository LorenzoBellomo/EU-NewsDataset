...
random.seed(42)
sample = random.sample([x for x in df_en[['maintext', 'id']].to_numpy()], 100000)
documents = {str(idx): {'text': row[0][:512], 'id': row[1]} for idx, row in enumerate(sample)}
print(len(documents), "total english documents")

# preprocessing only english documents
sp = WhiteSpacePreprocessing([a['text'] for a in documents.values()], stopwords_language='english')
...
print(topic_list)
with open("dump/topics_layer.json", "w") as json_file:
...
with open('dump/topic_dump.json', 'w') as json_file:
