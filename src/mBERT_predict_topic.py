...
def evaluate(dataloader_val, device, model, _field, epoch, model_type, _type):
    ...
    with open(f'/homenfs/l.bellomo1/EU-NewsDataset/models/{model_type}/{_field}/true_pred_class_epoch_{epoch}', "w") as f: # csv file with predictions (pred_class, true_class)
        writer = csv.writer(f)
        for row in rows:
            writer.writerow(row)
...
def train_model(df, model_type, _type, _field):
    ...
    assigned_labels = {'coronavirus': 1, 'economy': 0, 'entertainment': 2, 'news': 4, 'opinion': 3, 'politics': 5, 'science': 6, 'society': 7, 'sport': 8 }
    df['label'] = df['topic'].map(assigned_labels)
    print('len Dataset 9-class:', len(df)) # TODO: prova meno classi
    # dropping unecessary columns for classification
    df.drop(columns=['date_publish','language','source_domain','id','country','source_name', 'year','political_leaning'], inplace=True) # TODO: prova mergendo maintext con title e solo title
    print('n_articles for each label:', df['topic'].value_counts())
    ...
    # label dict 
    label_dict = pd.Series(df.label.values,index=df.topic).to_dict()
    print(label_dict)
    ...
    for epoch in range(1, epochs+1):
        ...
        # TODO change path
        torch.save(model.state_dict(), f'/homenfs/l.bellomo1/EU-NewsDataset/models/{model_type}/{_field}/finetuned_MBERT6class_epoch_{epoch}.model')
    ...
    print(df_stats)
    df_stats.to_csv(f'/homenfs/l.bellomo1/EU-NewsDataset/models/{model_type}/{_field}/MBERT_training_stats.csv')    # plot Learning Curve
    ...
    plt.legend()
    plt.xticks([1, 2, 3])
    plt.savefig(f'/homenfs/l.bellomo1/EU-NewsDataset/models/{model_type}/{_field}/MBERT_learningcurve.png') 
    plt.show()
...
#df = df.groupby("topic", group_keys=False).apply(lambda x: x.sample(10))
...
### mBERT all_text (title + maintext)  training no punt
print('\n')
print('----------------------------------')
